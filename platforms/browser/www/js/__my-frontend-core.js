...
/**** Кривейшее делегирование клика в заголовки тикетов ****/
    //$('.go-location-link').click(function(){
    $('body').on('click', '.lk-tickets .subject-name', function(){
    //$('body').delegate('.go-location-link', 'click', function(){
        ...
        /*---- Заголовок раздела ----*/
        top_title = 'Отправить заявку'; //$('.section.active').attr('data-title');
        if( top_title ){
            $('#top-title2').html( top_title );
        }
        else{
...
/**** Авторизация ****/
    ...
    /*** Формирование тикетов ***/
                ...
                for(var key in all['tickets_subjects']){
                    ...
                    tickets_subjects_html += '<div class="subject-name" data-link="lk-subject-' + all['tickets_subjects'][key]['id'] + '">' + all['tickets_subjects'][key]['name'] + '</div>';
                }
                    ...
                    for(var k in tickets_subjects){
                        if( all['messages'][key]['subject'] == tickets_subjects[k] ){
                            if( all['messages'][key]['vopros_otvet']=='vopros' ){
                                ...
                                if( all['messages'][key]['img'].length > 0 ){
                                    tickets[ k ] += '<div class="clear"></div>';
                                    tickets[ k ] += '<img src="http://cx35469.tmweb.ru/uploads/' + all['messages'][key]['img'] + '" />';
                                }
                                tickets[ k ] += '</div>';
                            }
